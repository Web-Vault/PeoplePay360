...
function StructureForm({ item, rules, onClose, onSaved }) {
  const [form, setForm] = useState(
    item
      ? {
          ...item,
          ruleIds: item.ruleIds.map((x) => x._id),
        }
      : empty
  );
  ...
  const toggle = (id) => {
    setForm((old) => ({
      ...old,
      ruleIds: old.ruleIds.includes(id)
        ? old.ruleIds.filter((x) => x !== id)
        : [...old.ruleIds, id],
    }));
  };
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      if (item) {
        await updateSalaryStructure(item._id, payload);
      } else {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      >
        <div className="mb-5 flex justify-between">
          <div>
            ...
            <p className="text-sm text-slate-500">
              Choose the rules that apply to this group of employees.
            </p>
          </div>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <label className="text-sm font-medium sm:col-span-2">
            ...
            <textarea
              ...
              className="mt-1 min-h-20 w-full rounded-lg border p-2.5"
            />
          </label>
        </div>
        ...
        <div className="mt-4">
          ...
          <div className="mt-2 grid max-h-64 gap-2 overflow-auto rounded-xl border p-3 sm:grid-cols-2">
            {rules.map((rule) => (
              <label
                key={rule._id}
              ...
              >
                <input
                  type="checkbox"
                  checked={form.ruleIds.includes(rule._id)}
                  onChange={() => toggle(rule._id)}
                />
...
export default function SalaryStructures() {
  ...
  const load = async () => {
    try {
      const [structures, salaryRules] = await Promise.all([
        listSalaryStructures(),
        listSalaryRules(),
      ]);

      setItems(structures.data.structures || []);
      setRules(salaryRules.data.rules || []);
    } catch (e) {
      setError(
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Bundle salary rules into reusable payroll structures.
          </p>
        </div>
          ...
          >
            ...
            <div className="mt-4 border-t pt-3 text-sm">
              ...
              <span className="ml-3 text-slate-500">
                {item.ruleIds.length} salary rules
              </span>
            </div>
          </section>
        ))}
