...
import { roleLabels, ROLES } from '../utils/constants'
...
function EmployeeDashboard({
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-2xl bg-slate-900 p-7 text-white shadow-lg">
        <p className="text-sm text-blue-200">Your work overview</p>
        ...
        <p className="mt-2 text-sm text-slate-300">
          Your attendance, overtime, leave, pay, and contract information.
        </p>
      </section>
      ...
      {expired && (
        <section className="flex gap-3 rounded-2xl border border-red-200 bg-red-50 p-5 text-red-800">
          <AlertTriangle className="h-5 w-5 shrink-0" />

          <div>
            <b>Contract action needed</b>

            <p className="mt-1 text-sm">
              Your current contract is expired or unavailable. Contact HR
      ...
      <section className="grid gap-4 sm:grid-cols-3">
        <MetricCard
          icon={Clock3}
          label="Worked today"
          value={`${summary?.today?.workedHours || 0} hrs`}
        ...
        <MetricCard
          ...
          value={`${summary?.today?.overtimeHours || 0} hrs`}
          detail="Cash or comp time eligible"
          tone="amber"
        />
        ...
        <MetricCard
          ...
          value={formatINR(summary?.today?.dailyEarnings || 0)}
          detail="Based on issued pay"
          tone="emerald"
        />
      </section>
      ...
      <Link
        to="/my-contract"
        className="block rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition hover:border-primary-300 hover:shadow-md"
      >
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">
                {contract
                  ? `${contract.contractNumber} · ${formatINR(
                      contract.pay?.net
                    )} monthly net`
          ...
          <div className="mt-4 flex items-center justify-between">
            <div>
              <p className="font-medium text-slate-800">
                {formatDate(summary.latestPayslip.periodStart)} –{' '}
                {formatDate(summary.latestPayslip.periodEnd)}
              </p>

              <p className="text-sm text-slate-500">
                Deductions:{' '}
                {formatINR(summary.latestPayslip.totalDeductions)}
              </p>
            </div>

            <p className="text-xl font-bold text-slate-900">
              {formatINR(summary.latestPayslip.netSalary)}
            </p>
          </div>
        ) : (
    ...
    <div className="space-y-6 pb-5">
        ...
        <div className="relative flex flex-col justify-between gap-5 sm:flex-row sm:items-end">
          <div>
            ...
            <p className="mt-2 max-w-xl text-sm leading-6 text-slate-300">
              Here’s a clear view of your people operations and the
              latest payroll cycle.
            </p>
          </div>

          <div className="flex items-center gap-2 rounded-xl border border-white/10 bg-white/10 px-3 py-2 text-sm text-slate-200 backdrop-blur-sm">
        ...
        {loading ? (
          ...
          <>
            <MetricCard
              ...
              label="User accounts"
              value={users.length}
              detail={`${activeUsers} active account${
                activeUsers === 1 ? '' : 's'
              }`}
            />
            ...
            <MetricCard
              ...
              label="Active access"
              value={activeUsers}
              detail={`${users.length - activeUsers} account${
                users.length - activeUsers === 1 ? '' : 's'
              } inactive`}
              tone="emerald"
            />
            ...
            <MetricCard
              icon={Wallet}
              label="Net payroll"
              value={formatINR(payroll.net)}
              detail={`${payroll.employees} employees in current run`}
              tone="violet"
            />
            ...
            <MetricCard
              icon={FileCheck2}
              label="Payroll audit"
              value={`${payroll.auditScore}%`}
              detail={`${payroll.warnings} items need a review`}
              tone="amber"
            />
          </>
        )}
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm xl:col-span-3 sm:p-6">
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">
                Distribution of available platform accounts
              </p>
            </div>
            ...
            <div>
              ...
              <div className="h-2 overflow-hidden rounded-full bg-slate-100">
                <div
                  ...
                  style={{
                    width: `${
                      (payroll.deductions / payroll.gross) * 100
                    }%`,
                  }}
                />
              </div>
      ...
      <section className="grid grid-cols-1 gap-6 xl:grid-cols-5">
        <div className="rounded-2xl border border-slate-200 bg-white shadow-sm xl:col-span-3">
          <div className="flex items-center justify-between border-b border-slate-100 px-5 py-5 sm:px-6">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">
                Most recently created platform accounts
              </p>
            </div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm xl:col-span-2 sm:p-6">
          ...
          <div className="mt-6 space-y-3">
            <div className="flex gap-3 rounded-xl border border-amber-100 bg-amber-50/60 p-3">
              ...
              <p className="text-sm leading-5 text-amber-900">
                <strong>{payroll.warnings} payroll checks</strong>{' '}
                are waiting for review before validation.
              </p>
            </div>

            <div className="flex gap-3 rounded-xl border border-emerald-100 bg-emerald-50/60 p-3">
              <CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-emerald-600" />

              <p className="text-sm leading-5 text-emerald-900">
                No critical payroll issues were found in the latest
                audit.
              </p>
            </div>
          </div>
          ...
          <Link
            to="/payroll"
            className="mt-5 flex items-center justify-between rounded-xl bg-slate-900 px-4 py-3 text-sm font-semibold text-white transition hover:bg-slate-800"
          >
            Review payroll
