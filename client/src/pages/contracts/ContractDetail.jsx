...
function SalaryRow({ label, value, red = false }) {
  return (
    <div className="flex justify-between py-2 text-sm">
      ...
      <b className={red ? 'text-red-600' : 'text-slate-900'}>
        {money(value)}
      </b>
    </div>
  );
...
function SalaryLines({ title, items, onChange, tone }) {
  ...
  return (
    <section className="rounded-xl border border-slate-200 p-4">
      ...
      {items.map((item, index) => (
        <div
          key={index}
          className="mt-2 grid grid-cols-[1fr_120px_30px] gap-2"
        >
          <input
            required
            value={item.name}
...
function OneTimeAdjustments({ items, onChange }) {
  const update = (index, key, value) => {
    onChange(
      items.map((item, i) =>
        i === index
  ...
  return (
    <section className="rounded-xl border border-amber-200 bg-amber-50/40 p-4">
      <div className="flex justify-between gap-3">
        <div>
          <b>One-time payroll adjustment</b>

          <p className="mt-1 text-xs text-slate-500">
            Used only in the selected month, then normal salary
            resumes.
          </p>
        </div>
      ...
      {items.map((item, index) => (
        <div
          key={index}
          className="mt-3 grid gap-2 rounded-lg border border-amber-100 bg-white p-3 sm:grid-cols-[1fr_110px_110px_120px_30px]"
        >
          <input
            required
            value={item.name}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      >
        ...
        <p className="mt-1 text-sm text-slate-500">
          Creates a new active contract and keeps this contract
          as history.
        </p>
  ...
  const load = async () => {
    try {
      ...
      setForm({
        ...
        allowances:
          item.allowances || item.pay?.allowances || [],
        deductions:
          item.deductions || item.pay?.deductions || [],
  ...
  const save = async (event) => {
    ...
    try {
      await updateContract(id, {
        ...form,
        allowances: form.allowances.filter((item) =>
          item.name.trim()
        ),
        deductions: form.deductions.filter((item) =>
          item.name.trim()
        ),
        adjustments: form.adjustments.filter((item) =>
          item.name.trim()
        ),
      });
          ...
          {editing ? (
            ...
            >
              ...
              <section className="rounded-xl border border-blue-200 bg-blue-50/40 p-4">
                ...
                <p className="mt-1 text-xs text-slate-500">
                  These terms determine the daily limit used
                  for overtime.
                </p>
            ...
            <div className="mt-4 divide-y">
              ...
              <div className="flex justify-between py-3 font-bold">
                ...
                <span>{money(contract.pay?.gross)}</span>
              </div>

              {contract.pay?.deductions?.map(
                (item, index) => (
                  <SalaryRow
                    key={index}
                    label={item.name}
              ...
              <div className="flex justify-between py-3 font-bold text-emerald-700">
                <span>Net monthly pay</span>
                <span>{money(contract.pay?.net)}</span>
              </div>
            </div>
          )}
        </section>
            ...
            <div className="mt-4 space-y-3 text-sm">
              <p className="text-slate-500">
                ...
                <b className="text-slate-900">
                  {contract.workStartTime || '09:00'} –{' '}
                  {contract.workEndTime || '18:00'}
                </b>
              </p>
              ...
              <p className="text-slate-500">
                ...
                <br />
                <b className="text-slate-900">
                  {contract.breakMinutes ?? 60} minutes
                </b>
              </p>
